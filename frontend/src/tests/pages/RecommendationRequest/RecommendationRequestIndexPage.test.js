...
describe("RecommendationRequestIndexPage tests", () => {
    ...
    test("renders three recommendation requests correctly for regular user", async () => {
        ...
        const createRecommendationRequestButton = screen.queryByText("Create Recommendation Request");
        expect(createRecommendationRequestButton).not.toBeInTheDocument();

        const requesterEmail = screen.getByTestId(`${testId}-cell-row-0-col-requesterEmail`);
        expect(requesterEmail).toHaveTextContent("student1@ucsb.edu");

        const professorEmail = screen.getByTestId(`${testId}-cell-row-0-col-professorEmail`);
        expect(professorEmail).toHaveTextContent("professor1@ucsb.edu");

        const explanation = screen.getByTestId(`${testId}-cell-row-0-col-explanation`);
        expect(explanation).toHaveTextContent("explanation1");

        const dateRequested = screen.getByTestId(`${testId}-cell-row-0-col-dateRequested`);
        expect(dateRequested).toHaveTextContent("2025-05-09T23:19:21.971Z");

        const dateNeeded = screen.getByTestId(`${testId}-cell-row-0-col-dateNeeded`);
        expect(dateNeeded).toHaveTextContent("2025-05-09T23:19:21.971Z");

        const done = screen.getByTestId(`${testId}-cell-row-0-col-Done`);
        expect(done).toHaveTextContent("true");
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        await waitFor(() => { expect(mockToast).toBeCalledWith("RecommendationRequest with id 2 was deleted") });
