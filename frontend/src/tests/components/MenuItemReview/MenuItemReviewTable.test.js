...
describe("UserTable tests", () => {
  ...
  test("Has the expected column headers and content for ordinary user", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <MenuItemReviewTable reviews={menuItemReviewFixtures.threeMenuItemReviews} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>
  ...
  test("Delete button calls delete callback", async () => {
    ...
    const deleteButton = screen.getByTestId(`${testId}-cell-row-0-col-Delete-button`);
    expect(screen.getByTestId(`MenuItemReviewTable-cell-row-0-col-reviewerEmail`)).toHaveTextContent("sampleEmail2@gmail.com");
